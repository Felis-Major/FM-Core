...
namespace FM.Runtime.Systems.Save
{
	...
	public static class GameDataHandler
	{
		...
		private class ActiveDataInfo
		{
			...
			{
				for (var i = 0; i < registeredDataSavers.Count; i++)
				{
					DataSaver registeredDataSaver = registeredDataSavers[i];
		...
		public static async Task Save(DataPersistency dataPersistency)
		{
			...
			List<int> activeSceneIDs = activeDataInfo.GetActiveSceneIDs();

			// Load unregistered data from Scene ID files
			for (var i = 0; i < activeSceneIDs.Count; i++)
			{
				var sceneID = activeSceneIDs[i];

				// Find registered data for this scene ID and convert to containers
				List<DataSaverInfo> registeredData = _registeredDataSavers
				...
				if (File.Exists(filePath))
				{
					...
					List<DataSaverInfo> loadedDataSavers = await fileHandler.Read<List<DataSaverInfo>>();
					List<string> loadedGUIDs = dataToSave.ConvertAll(x => x.GUID);

					// Add all data that doesn't have a loaded GUID
					for (var j = 0; j < loadedDataSavers.Count; j++)
					{
						DataSaverInfo loadedDataSaver = loadedDataSavers[i];
		...
		/// </summary>
		/// <param name="dataPersistency">Persistency of the data</param>
		public static async Task Load(DataPersistency dataPersistency)
		{
			...
			for (var i = 0; i < activeSceneIDs.Count; i++)
			{
				...
				if (File.Exists(filePath))
				{
					...
					for (var j = 0; j < registeredDataSavers.Count; j++)
					{
						DataSaver registeredDataSaver = registeredDataSavers[j];
						for (var k = 0; k < loadedDataSavers.Count; k++)
						{
							DataSaverInfo loadedData = loadedDataSavers[k];
		...
		/// </summary>
		public static async Task MakeTemporaryDataPermanent()
		{
			List<string> tempFiles = await GetSaveFiles(DataPersistency.Temporary);
			...
			for (var i = 0; i < tempFiles.Count; i++)
			{
				...
				if (File.Exists(permanentFileName))
				{
					...
					List<DataSaverInfo> tempFileData = await tempFileHandler.Read<List<DataSaverInfo>>();

					var permanentFileHandler = new FileHandler(permanentFileName, FileHandlerType);
					List<DataSaverInfo> permanentFileData = await permanentFileHandler.Read<List<DataSaverInfo>>();

					// Remove matching data from permanent file
					for (var j = 0; j < tempFileData.Count; j++)
					{
						DataSaverInfo tempData = tempFileData[j];
		...
		/// </summary>
		public static async Task FlushTempData()
		{
			List<string> tempFiles = await GetSaveFiles(DataPersistency.Temporary);
		...
		/// </summary>
		public static async Task ClearAllSavedData()
		{
		...
		/// </summary>
		public static async Task ClearActiveSlotSavedData()
		{
		...
		/// <returns></returns>
		public static async Task SaveSettings(SaveDataContainer saveDataContainer)
		{
			var fileHandler = new FileHandler(SettingsDataPath, FileHandler.FileType.Json);
		...
		public static async Task<SaveDataContainer> LoadSettings()
		{
			var fileHandler = new FileHandler(SettingsDataPath, FileHandler.FileType.Json);
			SaveDataContainer result = await fileHandler.Read<SaveDataContainer>();
			return result;
		}
		...
		private static async Task<List<string>> GetSaveFiles(DataPersistency dataPersistency)
		{
